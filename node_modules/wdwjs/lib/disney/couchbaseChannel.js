...
function DoLongPoll() {
  ...
  return this.GetLastSequence().then(lastSequenceID => needle('POST', `${this.BaseURL}/_changes?feed=longpoll&heartbeat=30000&style=all_docs&since=${lastSequenceID}&filter=sync_gateway%2Fbychannel`, {
    ...
    if (resp.body.results.length > 0) {
      ...
      return this.ProcessDocChanges(resp.body.results).then(() => {
        ...
        return this.SetLastSequence(resp.body.last_seq).then(() => {
          ...
          setTimeout(this[sLongPollFunction], this[sLongPollDelay] * 1000);
    ...
    // emit error
    this.emit('error', `Long-Polling error: ${err} (starting long-poll again in ${this[sLongPollErrorTimeout]} seconds}`);

    // restart long-poll for more changes in 10 seconds
    setTimeout(this[sLongPollFunction], this[sLongPollErrorTimeout] * 1000);
  });
}
