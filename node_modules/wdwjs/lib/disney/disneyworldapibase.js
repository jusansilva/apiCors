...
class DisneyPark extends Park {
  ...
  GetFacilityNames(facilityIDs) {
    return this.FacilityChannel.GetFacilitiesData(facilityIDs, {
      ...
      facilityDocs.forEach((facility) => {
        rideNames[DisneyUtil.CleanID(facility.id)] = facility.name;
      });
  ...
  FetchWaitTimes() {
    ...
    return this.FacilityStatusChannel.Start().then(() => this.FacilityStatusChannel.GetAllDocuments().then((docs) => {
      ...
      return this.GetFacilityNames(docsToFetch).then((facilityNames) => {
        Object.keys(facilityNames).forEach((facilityId) => {
          this.UpdateRide(facilityId, {
            name: facilityNames[facilityId],
