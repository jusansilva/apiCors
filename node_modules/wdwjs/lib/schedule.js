...
class Schedule {
  ...
  SetDate({
    ...
    if (!specialHours) {
      ...
      for (let i = 0; i < specialDayArray.length; i += 1) {
        if (JSON.stringify(specialDayArray[i]) === newDataStringified) {
  ...
  SetRange({
    ...
    const newOpeningTime = parseDateTime(openingTime, 'openingTime');
    const newClosingTime = parseDateTime(closingTime, 'closingTime');
  ...
  GetDate({
    ...
    if (this[sScheduleDatesSpecial].has(day)) {
      returnObject.special = this[sScheduleDatesSpecial].get(day);
    }
